...
const mimeTypes = { 
    '.html': 'text/html', 
    '.mp3': 'audio/mpeg' //mime тип для файлов формата .mp4 
}; 
...
http.createServer((request, response) => {// вызов метода создания http сервера   let pathname, extname, mimeType; 
    ...
    if (extname === '.mp3') { 
      ...
      let maxsize = 1024*500; /*Максимальный размер частичного контента 10KB (это значение выбрано для 
      ...
      responseHeaders['Content-Type'] = mimeType; 
      responseHeaders['Accept-Ranges'] = 'bytes'; //Говорим, что отдаем диапазон с байтами responseHeaders['Cache-Control'] = 'no-cache'; //не кешировать результат 
